  ...
  <div>
    <div id="hero" class="w-full bg-hero bg-cover bg-no-repeat bg-center"
      ...
      <div class="w-full absolute -mt-32 md:-mt-12 z-20 px-4 left-0">
        <div class="w-full max-w-5xl mx-auto">
          <div
            class="bg-white rounded-2xl shadow-xl p-4 md:p-6 flex flex-col md:flex-row items-center md:items-end gap-4 md:gap-6 border border-slate-50">

            <!-- Input Container -->
            <div class="flex-1 w-full space-y-2">
              <!-- Localized Label -->
              <label class="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1 md:hidden">
                {{ $t('packages.search_label') }}
              </label>
              <div class="relative group">
                <input v-model="search" type="text" :placeholder="$t('packages.search_placeholder')"
            ...
            <button
              ...
              <span>{{ $t('packages.search_button') }}</span>
            </button>
          </div>
        </div>
      </div>
    ...
    <div v-else class="flex flex-col items-center justify-center py-20 text-center">
      <p class="text-lg font-semibold text-muted-foreground">
        {{ $t('packages.no_results_title') }}
      </p>
      <p class="text-sm text-muted-foreground mt-2">
        {{ $t('packages.no_results_subtitle') }}
      </p>
    </div>
    ...
    <div class="w-full text-center my-10">
      <button @click="getTrips(search, data?.length + 10)"
        class="font-medium rounded-xl text-center border border-[#082852] border-5 p-3 hover:bg-slate-50 transition-colors">
          ...
          <div class="flex flex-col gap-6 mb-10 relative">
            ...
            <div v-for="(step, i) in steps" :key="i"
              ...
              <span class="font-bold text-[#082852] text-base md:text-lg group-hover:text-primary transition-colors">
                <!-- Ensure your steps data objects use the translation keys -->
                {{ $t(step.label) }}
              </span>
            </div>
          </div>
        ...
        <div class="flex-1 relative animate-fade-in-right">
          ...
          <div
            ...
            <div class="text-xs font-bold text-slate-700 uppercase tracking-tight">
              {{ $t('build_package.instant_booking') }} <br> {{ $t('build_package.booking_sub') }}
            </div>
          </div>
        </div>
...
const steps = [
  { label: 'build_package.steps.days', icon: Calendar },
  { label: 'build_package.steps.trips', icon: MapPin },
  { label: 'build_package.steps.create', icon: BadgePercent }
];
onMounted(() => {
  getTrips()
})
