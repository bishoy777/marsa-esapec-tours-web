    ...
    <div>
                    ...
                    <div
                        ...
                        <div class="relative flex-1 w-full">
                            <input v-model="search" type="text" 
                                :placeholder="$t('all_trips.search_placeholder')"
                        ...
                        <button @click="getTrips(search)"
                            ...
                            <span>{{ $t('all_trips.search_button') }}</span>
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div v-else class="flex flex-col items-center justify-center py-20 text-center">
            <p class="text-lg font-semibold text-muted-foreground">
                {{ $t('all_trips.no_results') }}
            </p>

            <p class="text-sm text-muted-foreground mt-2">
                {{ $t('all_trips.no_results_sub') }}
            </p>
        </div>

        <!-- Pagination -->
        <div class="w-full text-center my-10" v-if="data?.length">
            <button @click="getTrips(search, data.length + 10)"
                class="font-medium rounded-xl text-center border border-[#082852] border-5 p-3 transition-colors hover:bg-[#082852] hover:text-white">
                {{ $t('all_trips.load_more') }}
            </button>
        </div>
    </div>
</template>
